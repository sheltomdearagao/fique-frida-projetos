...
export default function ProductCarousel({
  ...
  return <div className="relative group mb-8 lg:mb-12">
      ...
      }} className="flex gap-4 lg:gap-6 overflow-x-auto hide-scrollbar px-4 lg:px-8 pb-6 py-[68px]">
          {products.map(product => <div key={product.id} onClick={() => onProductClick(product)} className="flex-none w-[280px] sm:w-[300px] lg:w-[320px] xl:w-[350px] bg-gray-900/95 rounded-xl overflow-hidden cursor-pointer transform transition-all duration-300 hover:scale-105 hover:z-10 hover:shadow-2xl hover:shadow-frida-magenta/20 group/card">
              {/* Imagem do Produto */}
              <div className="relative aspect-[4/3] overflow-hidden">
                <img src={product.image_urls?.[0] || ''} alt={product.name} className="w-full h-full object-cover transition-transform duration-500 group-hover/card:scale-110" onError={e => {
